...
| — | — | — | **Correction (2026-09-23, repo hygiene)** | PERF-01's `waitlist.html:16` evidence is stale: the waitlist was migrated to the shared design system (commit `ab44f55`) and no longer carries an inline `@font-face` or a `noto-serif-var.woff2` preload. `grep` finds no reference to `noto-serif` anywhere in HTML/CSS, and it is not in the `scripts/subset_fonts.py` master list; the 285 KB file was removed as dead. The only valid font preload remains `index.html:22` → `newsreader-roman-var.subset.woff2`. | — | — | — | — |
